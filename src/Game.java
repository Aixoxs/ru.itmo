...
public class Game {
    ...
    public Game(Place apartments) {
        if (apartments instanceof Apartments) {
            this.apartments = (Apartments)apartments;
        }
    }
    ...
    public void startGame() throws InterruptedException {
        ...
        characters.get(0).setLastRoom(((Apartments)apartments).getRoom(0));
        System.out.println(characters.get(0).getLastRoom().getName() + " стала началом погони");
        System.out.println("Впереди " + ((MoveAction)characters.get(0)).getAct() + " " + characters.get(0).getName());
        Thread.sleep(1000);
        ...
        for (int i = 1; i < characters.size() * 10; i++) {
            ...
            if (characters.get(number).getFirst()) {
                characters.get(number).setLastRoom(((Apartments)apartments).getCommunication(characters.get(0).getLastRoom()));
                System.out.println("Погоня переместилась и ее продолжением стала " + characters.get(0).getLastRoom().getName());
                System.out.println("Впереди все еще " + ((MoveAction)characters.get(0)).getAct() + " " + characters.get(0).getName() + characters.get(0).getLastAct());
                Thread.sleep(1000);
            }
            //действие последнего в игре и изменение расстояний
            else if (characters.get(number).getLast()) {
                System.out.println("Последним " + ((MoveAction)characters.get(number)).getAct() + " " + characters.get(number).getName() + characters.get(number).getLastAct());
                Thread.sleep(1000);
            ...
            //Два варианта для игроков, которые не являются ни последними, ни перавыми. В обоих случаях расстояние изменяется
                ...
                System.out.println(characters.get(number).getName() + " " + ((MoveAction)characters.get(number)).getAct() + " следом" + characters.get(number).getLastAct());
                Thread.sleep(1000);
                characters.get(number).setDistanceToTarget(characters.get(number).getDistanceToTarget() + characters.get(number-1).getSpeed() - characters.get(number).getSpeed());
            }
            else {
                System.out.println(characters.get(number).getName() + " " + ((MoveAction)characters.get(number)).getAct() + " за ним" + characters.get(number).getLastAct());
                Thread.sleep(1000);
                characters.get(number).setDistanceToTarget(characters.get(number).getDistanceToTarget() + characters.get(number-1).getSpeed() - characters.get(number).getSpeed());
            }
